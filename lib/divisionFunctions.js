mathVisual.fractionDivisionBar = (
  ...
  if (dividend.wholeNum == 0) {
    segmenter = (width / divisor.denominator) * divisor.numerator;
  } else {
    segmenter = (interval / divisor.denominator) * divisor.numerator;
  }

  let slicesToBeFilled =
    dividend.wholeNum * divisor.denominator +
    Math.floor(
      dividend.numerator / dividend.denominator / (1 / divisor.denominator)
    );

  let numSegments = Math.floor(slicesToBeFilled / divisor.numerator) + 1;
  let sizeOfLastPiece =
  ...
  for (let i = 1; i <= numSegments; i++) {
    ...
    dottedRect.setAttribute("stroke-dasharray", "5, 6");
  ...
  if (dividend.wholeNum == 0) {
    ...
    unitFractionWidth = width / (numSections * divisor.denominator);
  }
...
mathVisual.fractionDivisionCircles = (
  ...
  colorFill3 = "#b7dd82" // faded light green
) => {
  ...
  if (
    ...
    return alert(`All denominators must be positive.`);
  } else if (dividend.wholeNum * divisor.denominator > 300) {
    return alert(
      `These pieces will be too small to display nicely. Please try a smaller whole number or a smaller denominator.`
    );
  }
  ...
  // let quotientFloor = Math.floor(slicesToBeFilled / divisor.numerator);
  // console.log(
  ...
  for (let i = 0; i < slicesToBeFilled; i++) {
    if (i == 0) {
      ...
      startAngle += (1 / divisor.denominator) * Math.PI * 2;
    }
