mathVisual.fractionBar = (
  ...
  const width = rect.width - lineThickness - 5;
  const height = rect.height - lineThickness - 5;
...
mathVisual.fractionCircle = (
  ...
  if (mixedNum.numerator > mixedNum.denominator || mixedNum.wholeNum > 0) {
    ...
    console.log("mixedNumCircles was called.");
    return;
  }
  debugger;
  let radius = 0;
  ...
  drawCircle(svg, x, y, radius - lineThickness, lineThickness); // Draws the outer circle
...
mathVisual.mixedNumBars = (
  ...
  debugger;
  console.log(`maxWholes is ${maxWholes}`);
